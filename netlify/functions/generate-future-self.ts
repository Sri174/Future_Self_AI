...
export const handler: Handler = async (event) => {
  ...
  try {
    const { photoDataUri, interests, mindset, suggestedProfession, gender, userAge } = JSON.parse(event.body || '{}');
    ...
    try {
      ...
      const imagePromptText = photoDataUri
        ...
        **Critical Instructions:**
        1. **Age Progression:** Show the person as a ${futureAge}-year-old professional. They should look mature, experienced, and successful in their career, but maintain their core facial features and identity from the original photo.

        2. **Preserve Identity:** Meticulously preserve the person's distinct facial features, likeness, ethnicity, and bone structure while aging them appropriately to ${futureAge} years old. The generated person MUST be clearly and unmistakably identifiable as the person in the original photo, just older and more mature.
